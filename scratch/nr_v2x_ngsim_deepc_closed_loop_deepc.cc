...
class KpiPacketTag : public Tag
{
  public:
    /*
     * Purpose: Register the custom ns-3 packet tag that carries KPI metadata.
     * Inputs: none.
     * Output: ns-3 TypeId for KpiPacketTag.
     * Used by: ns-3 object system when packet tags are serialized/deserialized.
     */
    ...
    /*
     * Purpose: Return this tag instance type to ns-3.
     * Inputs: none.
     * Output: TypeId from GetTypeId().
     * Used by: ns-3 packet-tag handling.
     */
    ...
    /*
     * Purpose: Report the serialized byte size of the KPI tag.
     * Inputs: none.
     * Output: 20 bytes: sequence, Tx time, and Tx vehicle id.
     * Used by: ns-3 packet serialization.
     */
    ...
    /*
     * Purpose: Write KPI metadata into the packet tag buffer.
     * Inputs: TagBuffer i supplied by ns-3.
     * Output: no return; writes sequence, Tx time, and Tx vehicle id.
     * Used by: ns-3 when transmitting tagged CAM packets.
     */
    ...
    /*
     * Purpose: Read KPI metadata back from the packet tag buffer.
     * Inputs: TagBuffer i supplied by ns-3.
     * Output: no return; updates this tag instance fields.
     * Used by: ns-3 when receiving tagged CAM packets.
     */
    ...
    /*
     * Purpose: Print tag contents for debugging.
     * Inputs: output stream os.
     * Output: no return; writes a human-readable tag summary.
     * Used by: ns-3 logging/debug utilities.
     */
    ...
    /*
     * Purpose: Store the KPI sequence number for this CAM packet.
     * Inputs: sequence generated by CamApplication::TagAndLogTx.
     * Output: no return; updates m_sequence.
     * Used by: CamApplication::TagAndLogTx.
     */
    ...
    /*
     * Purpose: Read the KPI sequence number from a received packet tag.
     * Inputs: none.
     * Output: sequence number.
     * Used by: CamApplication::ReceiveCam.
     */
    ...
    /*
     * Purpose: Store the ns-3 transmit timestamp in nanoseconds.
     * Inputs: txTimeNs from Simulator::Now().GetNanoSeconds().
     * Output: no return; updates m_txTimeNs.
     * Used by: CamApplication::TagAndLogTx.
     */
    ...
    /*
     * Purpose: Read the stored transmit timestamp.
     * Inputs: none.
     * Output: transmit time in nanoseconds.
     * Used by: available for KPI/debug inspection.
     */
    ...
    /*
     * Purpose: Store the logical NGSIM vehicle id that transmitted the CAM.
     * Inputs: vehicleId assigned to the transmitting application.
     * Output: no return; updates m_txVehicleId.
     * Used by: CamApplication::TagAndLogTx.
     */
    ...
    /*
     * Purpose: Read the logical NGSIM vehicle id from the received CAM tag.
     * Inputs: none.
     * Output: transmitting vehicle id.
     * Used by: CamApplication::ReceiveCam.
     */
...
/*
 * Purpose: Compute Euclidean distance in the x-y plane.
 * Inputs: two ns-3 Vector positions a and b.
 * Output: distance in meters.
 * Used by: KPI eligibility checks and VDP distance calculations.
 */
static double
...
/*
 * Purpose: Clamp and round a floating-point value into a long integer range.
 * Inputs: value, minValue, maxValue.
 * Output: rounded long in [minValue, maxValue].
 * Used by: CAM latitude, longitude, speed, acceleration, and heading encoding.
 */
static long
...
/*
 * Purpose: Convert local y coordinate to ETSI CAM latitude units.
 * Inputs: yM local y position in meters.
 * Output: latitude encoded in 1e-7 degree integer units.
 * Used by: NgsimVehicleDataProvider CAM/position methods.
 */
static long
...
/*
 * Purpose: Convert local x coordinate to ETSI CAM longitude units.
 * Inputs: xM local x position in meters.
 * Output: longitude encoded in 1e-7 degree integer units.
 * Used by: NgsimVehicleDataProvider CAM/position methods.
 */
static long
...
/*
 * Purpose: Test whether a scalar coordinate lies inside the core corridor.
 * Inputs: coordinate, coreMin, coreMax.
 * Output: true when coordinate is within [coreMin, coreMax].
 * Used by: KPI filtering and peak-core vehicle counting.
 */
static bool
...
/*
 * Purpose: Place inactive vehicles far outside the simulated roadway.
 * Inputs: physical ns-3 node id.
 * Output: parking Vector outside communication range.
 * Used by: dynamic UE pool mobility and inactive waypoint padding.
 */
static Vector
...
/*
 * Purpose: Map physical ns-3 node id to currently assigned logical vehicle id.
 * Inputs: nodeId.
 * Output: vehicle id, or kInvalidVehicleId if unassigned.
 * Used by: KPI logger, CAM app assignment, and dynamic pool logic.
 */
static uint32_t
...
/*
 * Purpose: Map logical vehicle id to currently assigned physical ns-3 node id.
 * Inputs: vehicleId.
 * Output: node id, or max uint32_t if unassigned.
 * Used by: KpiLogger::LogRx and dynamic pool logic.
 */
static uint32_t
...
/*
 * Purpose: Check that a physical node is still assigned to a specific vehicle.
 * Inputs: nodeId and vehicleId.
 * Output: true if the current node-to-vehicle mapping matches.
 * Used by: Tx/Rx KPI filters to avoid stale dynamic-pool assignments.
 */
static bool
...
/*
 * Purpose: Decide whether a physical UE is active at the current simulation time.
 * Inputs: nodeId and timeS.
 * Output: true for assigned dynamic-pool nodes or in-range static nodes.
 * Used by: KPI counting, Tx/Rx filtering, and CBR node evaluation.
 */
static bool
...
/*
 * Purpose: Compute the first and last timestamps in one vehicle trajectory.
 * Inputs: rows for one logical vehicle.
 * Output: pair(minTimeS, maxTimeS).
 * Used by: data validation and dynamic-pool assignment scheduling.
 */
...
/*
 * Purpose: Compute total time coverage for a selected vehicle set.
 * Inputs: vehicleIds to evaluate.
 * Output: pair(minTimeS, maxTimeS) over all selected tracks.
 * Used by: validation-only mode and full-data coverage checks.
 */
static std::pair<double, double>
...
/*
 * Purpose: Find peak active vehicles and peak active core vehicles.
 * Inputs: vehicleIds plus optional core-filter settings.
 * Output: pair(peakActive, peakCore).
 * Used by: main() to size/check the physical UE pool and metadata.
 */
...
/*
 * Purpose: Compute time coverage of the PRBS input schedule.
 * Inputs: none; reads g_inputSchedule.
 * Output: pair(minTimeS, maxTimeS).
 * Used by: ValidateTenMinuteDataCoverage when useInputSchedule=true.
 */
static std::pair<double, double>
...
/*
 * Purpose: Verify that mobility and optional input data cover the evaluation run.
 * Inputs: simulation/evaluation times, schedule flag, coverage flag, vehicleIds.
 * Output: no return; aborts on insufficient coverage.
 * Used by: main() before building NR devices.
 */
static void
...
/*
 * Purpose: Compute min/max roadway coordinates along one axis.
 * Inputs: axis = "x" or "y".
 * Output: pair(minCoordinate, maxCoordinate).
 * Used by: ResolveCoreAxis and main() core-region setup.
 */
static std::pair<double, double>
...
/*
 * Purpose: Select the longitudinal core-filter axis.
 * Inputs: requestedAxis = "auto", "x", or "y".
 * Output: resolved axis string.
 * Used by: main() before computing core bounds.
 */
static std::string
...
/*
 * Purpose: Extract the configured core-filter coordinate from a 3D position.
 * Inputs: position and axis = "x" or "y".
 * Output: position.x or position.y.
 * Used by: KPI filtering and CBR node evaluation.
 */
static double
...
class NgsimPoolMobilityModel : public MobilityModel
{
  public:
    /*
     * Purpose: Register reusable NGSIM pool mobility with ns-3.
     * Inputs: none.
     * Output: ns-3 TypeId for NgsimPoolMobilityModel.
     * Used by: ns-3 object creation in main().
     */
    static TypeId GetTypeId()
    {
    ...
    /*
     * Purpose: Set where an unassigned physical UE is parked.
     * Inputs: inactive parking position.
     * Output: no return; updates m_parkingPosition.
     * Used by: main() when creating dynamic-pool UE mobility.
     */
    ...
    /*
     * Purpose: Attach a logical NGSIM vehicle trajectory to this physical UE.
     * Inputs: vehicleId and pointer to that vehicle's mobility rows.
     * Output: no return; mobility queries begin interpolating this trajectory.
     * Used by: AssignVehicleToPool.
     */
    ...
    /*
     * Purpose: Detach the current logical vehicle from this physical UE.
     * Inputs: none.
     * Output: no return; mobility returns to parked inactive position.
     * Used by: ReleaseVehicleFromPool.
     */
    void ReleaseVehicle()
    {
    ...
    /*
     * Purpose: Report whether this physical UE currently has a logical vehicle.
     * Inputs: none.
     * Output: true when a trajectory is assigned.
     * Used by: available for dynamic-pool inspection.
     */
    ...
    /*
     * Purpose: Return the logical vehicle currently assigned to this mobility model.
     * Inputs: none.
     * Output: vehicle id, or kInvalidVehicleId if unassigned.
     * Used by: available for dynamic-pool inspection.
     */
  ...
  private:
    /*
     * Purpose: Interpolate assigned NGSIM position at an arbitrary simulation time.
     * Inputs: timeS in seconds.
     * Output: interpolated position, or parking position if unassigned.
     * Used by: DoGetPosition.
     */
    ...
    /*
     * Purpose: Interpolate assigned NGSIM velocity at an arbitrary simulation time.
     * Inputs: timeS in seconds.
     * Output: interpolated velocity, or zero velocity if unassigned.
     * Used by: DoGetVelocity.
     */
    ...
    /*
     * Purpose: Provide current position to ns-3 mobility users.
     * Inputs: none; reads Simulator::Now().
     * Output: current interpolated position.
     * Used by: ns-3 mobility framework, KPI logger, VDP, and CBR filters.
     */
    ...
    /*
     * Purpose: Set the parked position for this mobility model.
     * Inputs: position.
     * Output: no return; updates m_parkingPosition and notifies ns-3.
     * Used by: ns-3 mobility framework if position is manually set.
     */
    ...
    /*
     * Purpose: Provide current velocity to ns-3 mobility users.
     * Inputs: none; reads Simulator::Now().
     * Output: current interpolated velocity.
     * Used by: VDP and ns-3 mobility framework.
     */
...
/*
 * Purpose: Load processed NGSIM trajectory rows from CSV.
 * Inputs: path to mobility CSV.
 * Output: no return; fills g_mobilityByVehicle sorted by time.
 * Used by: main() before selecting vehicles and installing mobility.
 */
static void
...
/*
 * Purpose: Load open-loop PRBS input schedule from CSV.
 * Inputs: path to input schedule CSV.
 * Output: no return; fills g_inputSchedule.
 * Used by: main() only when useInputSchedule=true.
 */
static void
...
/*
 * Purpose: Install static waypoint mobility for one logical vehicle.
 * Inputs: physical node and that vehicle's mobility rows.
 * Output: no return; aggregates WaypointMobilityModel onto the node.
 * Used by: main() when dynamicActivePool=false.
 */
static void
...
/*
 * Purpose: Parse an NR sidelink slot bitmap string into bitset tokens.
 * Inputs: slBitMapString such as "1|1|0", output vector reference.
 * Output: no return; appends parsed bitmap entries to slBitMapVector.
 * Used by: main() during sidelink resource-pool configuration.
 */
static void
...
class KpiLogger : public Object
{
  public:
    /*
     * Purpose: Initialize KPI logging, evaluation filters, and output CSV.
     * Inputs: UE nodes, CBR logger, CSV path, KPI window/sample times, evaluation
     *         window, and core-region filter configuration.
     * Output: no return; opens KPI CSV and stores logger configuration.
     * Used by: main() after NR devices, IP stack, and CBR logger are configured.
     */
    ...
    /*
     * Purpose: Register the IPv4 address used by one ns-3 UE node.
     * Inputs: nodeId and assigned IPv4 address.
     * Output: no return; updates IP-to-node lookup map.
     * Used by: main() after assigning UE IPv4 addresses.
     */
    ...
    /*
     * Purpose: Convert a received packet source IPv4 address to an ns-3 node id.
     * Inputs: source IPv4 address.
     * Output: node id, or max uint32_t if the IP is unknown.
     * Used by: CamApplication::ReceiveCam.
     */
    ...
    /*
     * Purpose: Store the currently applied communication inputs for KPI output.
     * Inputs: pDbm transmit power and tbS beacon interval.
     * Output: no return; updates current input fields written to KPI CSV.
     * Used by: ApplyInputsAtTime, ApplyTxPowerAtTime, and main() initialization.
     */
    ...
    /*
     * Purpose: Log one transmitted CAM as a PRR denominator opportunity.
     * Inputs: Tx vehicle/node ids, sequence, Tx time, and Tx position.
     * Output: no return; appends a TxEvent with eligible receivers in range.
     * Used by: CamApplication::TagAndLogTx.
     */
    ...
    /*
     * Purpose: Log one received CAM as a PRR numerator and PIR observation.
     * Inputs: Tx/Rx vehicle ids, sequence, Rx time, and Rx position.
     * Output: no return; appends a unique RxEvent if it matches an eligible Tx.
     * Used by: CamApplication::ReceiveCam.
     */
    ...
    /*
     * Purpose: Compute and write one KPI sample over the sliding KPI window.
     * Inputs: none; reads current simulator time and logged Tx/Rx events.
     * Output: no return; writes CSV row and schedules the next sample.
     * Used by: main() through Simulator::Schedule.
     */
    void SampleAndWrite()
    {
        PruneOldEvents();
    ...
    /*
     * Purpose: Report whether at least one KPI sample has been computed.
     * Inputs: none.
     * Output: true if m_latestSample is valid.
     * Used by: RunDeepcBridgeStep before pushing history to DeePC.
     */
    ...
    /*
     * Purpose: Return the most recent KPI sample.
     * Inputs: none.
     * Output: latest KpiSample struct.
     * Used by: RunDeepcBridgeStep.
     */
    ...
    /*
     * Purpose: Close the KPI CSV when the logger is destroyed.
     * Inputs: none.
     * Output: no return; closes m_out if open.
     * Used by: ns-3 object lifetime management after simulation.
     */
    ~KpiLogger() override
    {
        if (m_out.is_open())
  ...
  private:
    /*
     * Purpose: Count currently active vehicles inside the core evaluation region.
     * Inputs: timeS for active-status check.
     * Output: active core vehicle count.
     * Used by: SampleAndWrite.
     */
    ...
    /*
     * Purpose: Print one concise progress line per simulated second.
     * Inputs: current time and active core vehicle count.
     * Output: no return; writes to stdout at most once per second.
     * Used by: SampleAndWrite.
     */
    ...
    /*
     * Purpose: Keep only Tx/Rx events that still belong to the KPI window.
     * Inputs: none; reads Simulator::Now().
     * Output: no return; prunes old events and stale duplicate-Rx keys.
     * Used by: LogTx, LogRx, and SampleAndWrite.
     */
    void PruneOldEvents()
    {
        const double now = Simulator::Now().GetSeconds();
    ...
    /*
     * Purpose: Test whether an event timestamp is inside the evaluation window.
     * Inputs: timeS.
     * Output: true when time is inside [m_evalStartS, m_evalEndS].
     * Used by: LogTx and LogRx.
     */
    ...
    /*
     * Purpose: Test whether a position passes the configured core-region filter.
     * Inputs: ns-3 position Vector.
     * Output: true if the position is accepted for KPI evaluation.
     * Used by: LogTx, LogRx, and CountActiveCoreVehicles.
     */
    ...
    /*
     * Purpose: Find a transmitted event by vehicle id and KPI sequence.
     * Inputs: txVehicleId and sequence number.
     * Output: pointer to TxEvent, or nullptr if not found.
     * Used by: LogRx before counting a reception.
     */
...
class NgsimVehicleDataProvider : public VDP
{
  public:
    /*
     * Purpose: Create a VDP wrapper around one ns-3 node mobility model.
     * Inputs: node that owns the CAM application.
     * Output: constructed provider with initial position/speed state.
     * Used by: CamApplication::StartApplication and ConfigureVehicleServiceState.
     */
    ...
    /*
     * Purpose: Build mandatory CAM fields from NGSIM mobility.
     * Inputs: none; reads node position and velocity.
     * Output: CAM_mandatory_data_t for CABasicService.
     * Used by: CABasicService when generating CAM packets.
     */
    ...
    /*
     * Purpose: Provide CPM mandatory data by reusing CAM state fields.
     * Inputs: none.
     * Output: CPM_mandatory_data_t.
     * Used by: VDP interface; not central to this CAM-only experiment.
     */
    ...
    /*
     * Purpose: Provide MCM mandatory data by reusing CAM state fields.
     * Inputs: none.
     * Output: MCM_mandatory_data_t.
     * Used by: VDP interface; not central to this CAM-only experiment.
     */
    ...
    /*
     * Purpose: Compute scalar vehicle speed from node velocity.
     * Inputs: none.
     * Output: speed in m/s.
     * Used by: CAM encoding and acceleration calculation.
     */
    ...
    /*
     * Purpose: Accumulate travelled distance from current position changes.
     * Inputs: none.
     * Output: cumulative distance in meters.
     * Used by: VDP interface.
     */
    ...
    /*
     * Purpose: Compute vehicle heading from velocity.
     * Inputs: none.
     * Output: heading in degrees, preserving last heading at near-zero speed.
     * Used by: CAM mandatory data.
     */
    ...
    /*
     * Purpose: Return WGS84-like position for VDP consumers.
     * Inputs: none.
     * Output: latitude/longitude converted from local NGSIM coordinates.
     * Used by: VDP interface.
     */
    ...
    /*
     * Purpose: Return local Cartesian position for VDP consumers.
     * Inputs: none.
     * Output: local x, y, z coordinates.
     * Used by: VDP interface.
     */
    ...
    /*
     * Purpose: Convert longitude/latitude back to local Cartesian coordinates.
     * Inputs: lon and lat in degrees.
     * Output: local x/y position.
     * Used by: VDP distance helper methods.
     */
    ...
    /*
     * Purpose: Compute local Cartesian distance between two lon/lat points.
     * Inputs: lon1, lat1, lon2, lat2 in degrees.
     * Output: distance in meters.
     * Used by: VDP interface.
     */
    ...
    /*
     * Purpose: Report lane-position availability.
     * Inputs: none.
     * Output: unavailable VDPDataItem.
     * Used by: VDP interface; lane position is not used in this experiment.
     */
    ...
    /*
     * Purpose: Report exterior-light availability.
     * Inputs: none.
     * Output: unavailable VDPDataItem.
     * Used by: VDP interface; exterior lights are not used in this experiment.
     */
  ...
  private:
    /*
     * Purpose: Read current node position from its ns-3 mobility model.
     * Inputs: none.
     * Output: current position Vector, or zero Vector if missing.
     * Used by: all VDP position/CAM methods.
     */
    ...
    /*
     * Purpose: Read current node velocity from its ns-3 mobility model.
     * Inputs: none.
     * Output: current velocity Vector, or zero Vector if missing.
     * Used by: speed, heading, and acceleration calculations.
     */
    ...
    /*
     * Purpose: Estimate longitudinal acceleration from speed difference.
     * Inputs: none; uses current speed and previous speed/time.
     * Output: ETSI acceleration value, or unavailable when not computable.
     * Used by: getCAMMandatoryData.
     */
...
class CamApplication : public Application
{
  public:
    /*
     * Purpose: Bind this CAM application to its node, KPI logger, multicast IP, and port.
     * Inputs: node, logger, group IPv4 address, UDP port.
     * Output: no return; stores application dependencies.
     * Used by: main() when creating one CAM app per physical UE.
     */
    ...
    /*
     * Purpose: Configure the fixed CAM generation interval.
     * Inputs: tb in seconds.
     * Output: no return; updates CABasicService generation limits.
     * Used by: main(), ApplyInputsAtTime, and fixed 10 Hz setup.
     */
    ...
    /*
     * Purpose: Select ETSI-style dynamic CAM generation or fixed periodic CAMs.
     * Inputs: enable flag.
     * Output: no return; stores CAM generation mode.
     * Used by: main() before starting applications.
     */
    ...
    /*
     * Purpose: Assign a logical vehicle id to this physical UE application.
     * Inputs: vehicleId from dynamic/static assignment.
     * Output: no return; updates station state and starts CAMs if ready.
     * Used by: main() and AssignVehicleToPool.
     */
    ...
    /*
     * Purpose: Remove the current logical vehicle from this application.
     * Inputs: none.
     * Output: no return; stops CAM dissemination and clears vehicle id.
     * Used by: ReleaseVehicleFromPool.
     */
    void ReleaseVehicle()
    {
    ...
    /*
     * Purpose: Return the logical vehicle currently assigned to the app.
     * Inputs: none.
     * Output: vehicle id, or kInvalidVehicleId.
     * Used by: available for assignment/debug inspection.
     */
  ...
  protected:
    /*
     * Purpose: Start sockets, GeoNet/BTP/CAM service, callbacks, and CAM dissemination.
     * Inputs: none; uses values set by Setup and assignment methods.
     * Output: no return; application enters running state.
     * Used by: ns-3 Application lifecycle at app start time.
     */
    ...
    /*
     * Purpose: Stop CAM dissemination and close the UDP socket.
     * Inputs: none.
     * Output: no return; application leaves running state.
     * Used by: ns-3 Application lifecycle at app stop time.
     */
  ...
  private:
    /*
     * Purpose: Attach KPI metadata to each outgoing CAM and log the Tx event.
     * Inputs: packet emitted by CABasicService.
     * Output: no return; packet receives KpiPacketTag and KpiLogger sees Tx.
     * Used by: CABasicService Tx packet callback.
     */
    ...
    /*
     * Purpose: Compute deterministic per-vehicle start offset to avoid synchronized CAMs.
     * Inputs: none; uses assigned vehicle id or node id.
     * Output: desynchronization delay in seconds.
     * Used by: StartCamIfReady before CAM dissemination begins.
     */
    ...
    /*
     * Purpose: Process received CAMs and log valid KPI receptions.
     * Inputs: decoded CAM, source address, and received packet.
     * Output: no return; KpiLogger may receive a LogRx call.
     * Used by: CABasicService Rx packet callback.
     */
    ...
    /*
     * Purpose: Keep CAM/BTP services synchronized with the current vehicle id and VDP.
     * Inputs: none.
     * Output: no return; updates station id and VDP pointers.
     * Used by: StartApplication, AssignVehicle, and ReleaseVehicle.
     */
    ...
    /*
     * Purpose: Start CAM dissemination once app is running and vehicle is assigned.
     * Inputs: none.
     * Output: no return; CABasicService starts CAM generation.
     * Used by: StartApplication and AssignVehicle.
     */
    ...
    /*
     * Purpose: Stop CAM dissemination if currently active.
     * Inputs: none.
     * Output: no return; updates m_disseminating.
     * Used by: StopApplication and ReleaseVehicle.
     */
...
/*
 * Purpose: Assign one logical NGSIM vehicle to an available physical UE.
 * Inputs: logical vehicleId.
 * Output: no return; updates global mapping, mobility model, and CAM app state.
 * Used by: AssignInitialDynamicPoolVehicles and ScheduleDynamicPoolAssignments.
 */
static void
...
/*
 * Purpose: Release one logical vehicle from its physical UE slot.
 * Inputs: logical vehicleId.
 * Output: no return; clears app/mobility/mapping state and returns node to free pool.
 * Used by: ScheduleDynamicPoolAssignments.
 */
static void
...
/*
 * Purpose: Assign vehicles already active at simulation start.
 * Inputs: selected vehicleIds and initialTimeS.
 * Output: no return; calls AssignVehicleToPool for active tracks.
 * Used by: main() when dynamicActivePool=true.
 */
static void
...
/*
 * Purpose: Schedule future vehicle assignment/release events for the dynamic pool.
 * Inputs: selected vehicleIds, initial time, and simulation end time.
 * Output: no return; schedules AssignVehicleToPool and ReleaseVehicleFromPool.
 * Used by: main() after CAM applications are created.
 */
static void
...
/*
 * Purpose: Apply one transmit-power value to every NR sidelink UE PHY.
 * Inputs: UE device container and pDbm.
 * Output: no return; updates TxPower on each UE BWP PHY.
 * Used by: main(), ApplyInputsAtTime, and ApplyTxPowerAtTime.
 */
static void
...
/*
 * Purpose: Apply open-loop scheduled Tx power and beacon interval.
 * Inputs: KPI logger, UE devices, Tx power pDbm, beacon interval tbS.
 * Output: no return; updates logger, apps, and PHY Tx power.
 * Used by: ScheduleInputUpdates when useInputSchedule=true.
 */
static void
...
/*
 * Purpose: Apply closed-loop DeePC Tx-power command while keeping 10 Hz beaconing fixed.
 * Inputs: KPI logger, UE devices, pDbm command, fixed beacon interval.
 * Output: no return; updates logger current inputs and UE Tx power.
 * Used by: RunDeepcBridgeStep.
 */
static void
...
/*
 * Purpose: Assign deterministic nonzero sidelink source L2 IDs to all UEs.
 * Inputs: UE device container.
 * Output: no return; updates NR/LTE sidelink RRC and MAC source IDs.
 * Used by: main() after UE device installation and after SL preconfiguration.
 */
static void
...
/*
 * Purpose: Schedule all PRBS open-loop input updates.
 * Inputs: KPI logger and UE devices.
 * Output: no return; schedules ApplyInputsAtTime for each input row.
 * Used by: main() only when useInputSchedule=true.
 */
static void
...
/*
 * Purpose: Connect NR PHY ChannelOccupied traces to the CBR logger.
 * Inputs: pointer to configured CbrLogger.
 * Output: no return; installs ns-3 Config callback.
 * Used by: main() before starting CBR logging.
 */
static void
...
/*
 * Purpose: Build metadata sidecar path from a CSV output path.
 * Inputs: csvPath and suffix such as "_metadata.json".
 * Output: derived path string.
 * Used by: WriteMetadataJson.
 */
static std::string
...
/*
 * Purpose: Escape a string so it can be safely written into JSON.
 * Inputs: raw string value.
 * Output: JSON-escaped string without surrounding quotes.
 * Used by: WriteMetadataJson.
 */
static std::string
...
/*
 * Purpose: Write a numeric vector as a JSON array.
 * Inputs: output stream and double vector.
 * Output: no return; appends JSON array text to stream.
 * Used by: DeepcFileBridge::WriteRequest.
 */
static void
...
/*
 * Purpose: Extract one numeric field from a simple controller response JSON.
 * Inputs: JSON text, field key, output double reference.
 * Output: true if parsed; value is updated on success.
 * Used by: DeepcFileBridge::RequestControl.
 */
static bool
...
/*
 * Purpose: Extract one boolean field from a simple controller response JSON.
 * Inputs: JSON text, field key, output bool reference.
 * Output: true if parsed; value is updated on success.
 * Used by: DeepcFileBridge::RequestControl.
 */
static bool
...
/*
 * Purpose: Extract one string field from a simple controller response JSON.
 * Inputs: JSON text, field key, output string reference.
 * Output: true if parsed; value is updated on success.
 * Used by: DeepcFileBridge::RequestControl.
 */
static bool
...
class DeepcFileBridge
{
  public:
    /*
     * Purpose: Initialize the JSON request/response bridge and diagnostic logs.
     * Inputs: bridge directory, horizons, timeout, Tx-power bounds, fixed beacon interval.
     * Output: no return; creates directories and opens applied-control/timing CSVs.
     * Used by: main() before scheduling RunDeepcBridgeStep.
     */
    ...
    /*
     * Purpose: Add a new KPI sample to the DeePC past-history buffer.
     * Inputs: latest KpiSample from KpiLogger.
     * Output: true if the sample was new and accepted.
     * Used by: RunDeepcBridgeStep.
     */
    ...
    /*
     * Purpose: Report whether enough KPI history exists for DeePC control.
     * Inputs: none.
     * Output: true when history length equals m_pastHorizon.
     * Used by: RunDeepcBridgeStep and RequestControl guard.
     */
    bool Ready() const
    {
    ...
    /*
     * Purpose: Return current number of stored history samples.
     * Inputs: none.
     * Output: history buffer size.
     * Used by: available for bridge/debug inspection.
     */
    ...
    /*
     * Purpose: Return configured DeePC past horizon.
     * Inputs: none.
     * Output: m_pastHorizon.
     * Used by: available for bridge/debug inspection.
     */
    ...
    /*
     * Purpose: Send one request to the external Python DeePC controller and read response.
     * Inputs: control step, simulation time, and previous applied input.
     * Output: InputCommand to apply; falls back to previous Tx power on invalid response.
     * Used by: RunDeepcBridgeStep.
     */
  ...
  private:
    /*
     * Purpose: Build deterministic request/response filename for one control step.
     * Inputs: directory, filename prefix, step number.
     * Output: path such as request_000123.json.
     * Used by: RequestControl.
     */
    ...
    /*
     * Purpose: Build DeePC past input vector from buffered KPI samples.
     * Inputs: none; reads m_samples.
     * Output: vector of tx_power_dbm values.
     * Used by: WriteRequest.
     */
    ...
    /*
     * Purpose: Build DeePC past output vector from buffered KPI samples.
     * Inputs: none; reads m_samples.
     * Output: vector ordered as PRR, PIR, CBR for each past sample.
     * Used by: WriteRequest.
     */
    ...
    /*
     * Purpose: Build DeePC past context vector from buffered KPI samples.
     * Inputs: none; reads m_samples.
     * Output: vector ordered as active_vehicle_count_core, beacon_interval_s.
     * Used by: WriteRequest.
     */
    ...
    /*
     * Purpose: Write one complete DeePC request JSON for the Python controller.
     * Inputs: request path, step/time, and previous input command.
     * Output: no return; atomically creates request_XXXXXX.json.
     * Used by: RequestControl.
     */
...
/*
 * Purpose: Periodically push KPI history to DeePC, request control, and apply Tx power.
 * Inputs: logger, UE devices, bridge, evaluation times, control interval,
 *         fixed beacon interval, previous command, and control step number.
 * Output: no return; schedules itself until evaluation end.
 * Used by: main() through Simulator::Schedule.
 */
static void
...
/*
 * Purpose: Write simulation configuration and metric definitions next to KPI CSV.
 * Inputs: output paths, simulation settings, radio settings, vehicle counts,
 *         DeePC bridge settings, and Tx-power bounds.
 * Output: no return; creates *_metadata.json sidecar file.
 * Used by: main() before Simulator::Run().
 */
static void
...
/*
 * Purpose: Build and run the complete closed-loop NR-V2X DeePC experiment.
 * Inputs: command-line arguments parsed by ns-3 CommandLine.
 * Output: process exit code; writes KPI, CBR, bridge, and metadata artifacts.
 * Used by: executable entry point for ns-3 run.
 */
int
