...
class DeepcFileBridge
{
  public:
                   ...
                   double timeoutS)
    {
        ...
        m_appliedOut << "step,time_s,tx_power_dbm,beacon_interval_s,response_success,"
                     << "controller_solve_time_s,response_path\n";
        ...
        m_timingOut << "step,time_s,wait_time_s,controller_solve_time_s,response_success\n";
    }
  ...
  private:
                      ...
                      const InputCommand& previousU) const
    {
        ...
        out << "  \"output_cols\": [\"prr_150m\", \"pir_s\", \"cbr\"],\n";
...
main(int argc, char* argv[])
{
    ...
    // File paths
    std::string mobilityCsv = "data/processed/ngsim_us101_10min_0p1s.csv";
    std::string inputCsv = "data/processed/prbs_schedule.csv";
    std::string kpiCsv = "data/output/kpi_timeseries_deepc_copy_run01.csv";
    std::string cbrCsv = "data/output/cbr_timeseries_deepc_copy_run01.csv";
    ...
    std::cout << "Copy scenario output guard = "
    ...
    NS_ABORT_MSG_IF(enableDeepcBridge && useInputSchedule,
                    "Closed-loop DeePC bridge cannot run together with PRBS input schedule. "
                    "Use --enableDeepcBridge=true --useInputSchedule=false.");
    NS_ABORT_MSG_IF(enableDeepcBridge && deepcControlIntervalS < sampleTimeS - 1e-9,
                    "deepcControlInterval must be at least sampleTime");
